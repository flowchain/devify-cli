...
program
.command('update <dir>')
.description('Update your project libraries')
.action(function(dir) {
  if(!test('-d', dir)) {
    console.log(chalk.red('failed: ' + dir + ' project directory not exits!'));
    exit(1);
  }
  ...
  var interval = setInterval(function() {
    logUpdate("Updating your project..." + chalk.cyan.bold.dim(frame()));
  }, 50)

  console.log(chalk.yellow.bold('Start to update your project: ' + program.args[0]));
  exec('git remote add devify https://github.com/DevifyPlatform/devify-server', function(code, stdout, stderr) {
    if(code !== 0) {
      console.log(chalk.red.bold('git: remote add error. Please try again!'));
      exit(1);
    }

    exec('git fetch devify', function(code, stdout, stderr) {
      if(code !== 0) {
        console.log(chalk.red.bold('git: remote fetch error. Please try again!'));
        exit(1);
      }

      exec('git checkout devify/master libs', function(code, stdout, stderr) {
        if(code !== 0) {
          console.log(chalk.red.bold('git: remote checkout error. Please try again!'));
          exit(1);
        }

        exec('git remote remove devify', function(code, stdout, stderr) {
          clearInterval(interval);
          if(code !== 0) {
            console.log(chalk.red.bold('git: remote remove error. Please try again!'));
            exit(1);
          }

          console.log(chalk.green.bold('Completed.'));
          console.log(chalk.yellow.bold('* Your project is updated.'));
        });
      });
    });
  });
